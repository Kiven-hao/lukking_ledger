    ...
    <div className="grid">
        ...
        <div className="page-header-top">
          <div>
            ...
            <p className="body-copy" style={{ margin: 0 }}>
              {ledger.description || "还没有账本说明。"}
            </p>
          </div>

          <div className="button-row">
            <Link
              href={`/dashboard/ledger/${id}/analytics`}
              className="action-chip"
            >
              查看分析
            </Link>
            <Link
              href={`/dashboard/ledger/${id}/settings`}
              className="action-chip"
            >
              账本设置
            </Link>
      ...
      <section className="stats-grid fade-up stagger-1">
        <div className="stat-card">
          <p style={{ margin: 0, color: "var(--text-secondary)" }}>本页收入</p>
        ...
        <div className="stat-card">
          <p style={{ margin: 0, color: "var(--text-secondary)" }}>本页支出</p>
        ...
        <div className="stat-card">
          <p style={{ margin: 0, color: "var(--text-secondary)" }}>本页转账</p>
          <strong style={{ color: "var(--transfer)" }}>
            {summary.transfer.toFixed(2)}
          </strong>
        </div>

        <div className="stat-card">
          <p style={{ margin: 0, color: "var(--text-secondary)" }}>成员数</p>
      ...
      >
        ...
        <h2 style={{ marginTop: 8, marginBottom: 4 }}>新增交易</h2>

        <p className="body-copy" style={{ marginTop: 0 }}>
          表单会根据交易类型联动分类，让录入动作更短、更顺。
        </p>
      ...
      >
        ...
        >
          <div>
            ...
            <h2 style={{ margin: "8px 0 0" }}>交易列表</h2>
            <p className="body-copy" style={{ margin: "8px 0 0" }}>
              支持按类型、分类、日期和备注关键字筛选，并使用 cursor 翻页。
            </p>
          </div>
        </div>
        ...
        {transactionError ? (
          <p className="alert-error">
            交易列表查询失败：{transactionError.message}
          </p>
        ) : null}

        {!transactionError && creatorsError ? (
          <p className="alert-error">
            用户信息查询失败：{creatorsError.message}
          </p>
        ) : null}

        <TransactionList items={transactionItems} currency={ledger.currency} />

        <div className="soft-divider" style={{ marginTop: 24 }} />

        <div
          className="button-row fade-up stagger-4"
